              ...
              <div class="columns large-3 medium-3 small-3" v-for="(post, index) in posts" v-bind:key="index">
                  <div class="card">
                    <div class="card-divider">
                        ...
                        <!-- {{ post.title }} -->
                        <h5 class="h5 g-color-gray-dark-v1 mb-0"><a v-if="post.source!= null" :href="post.source.split(': ')[1]" class="g-text-underline--none--hover">{{ post.source.split(": ")[0] || post.author.username }}</a> <span class="h6">Posted <router-link v-bind:to="{ name: 'EbookDetail', params: { id: post.id } }" class="g-text-underline--none--hover">《<span v-html="post.title"></span>》</router-link></span></h5>

                      </div>
                      <router-link v-bind:to="{ name: 'PostDetail', params: { id: post.id } }"> <img :src="require(`../assets/Elementary/${post.title}.jpg`)"></router-link>
...
import EbookBase from './Base/Ebook'
...
export default {
  name: 'HomeEbook',  //this is the name of the component
  components: {
    EbookBase,
    Pagination,
    Topstories,
  ...
  methods: {
    ...
    getPosts () {
      ...
      const path = `/api/posts/ebook/?page=${page}&per_page=${per_page}`
